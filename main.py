...
async def send_error(interaction: discord.Interaction, error: Exception | str) -> None:
    """Standard ephemeral error response."""
    msg = str(error)
    try:
...
async def resolve_character_input(
    ...
    """Resolve a character input.

    Accepts either:
      - token 'user_id:name' (preferred from autocomplete)
      - raw name (must be unique in guild)
    Returns (user_id, character_name).
    """
    token = (character_token or "").strip()
    ...
    exact = [r for r in rows if str(r["name"]).lower() == token.lower()]
    if len(exact) == 1:
        return int(exact[0]["user_id"]), str(exact[0]["name"])
    if len(exact) > 1:
        raise ValueError("Multiple characters share that name. Please choose from the autocomplete list.")
...
async def list_player_ids(self, guild_id: int) -> List[int]:
    """List player user_ids for a guild.

    Prefer the players table if it exists; otherwise fall back to distinct user_ids from characters.
    """
    try:
        rows = await self._fetchall(
        ...
        LOG.debug("players table missing/empty; falling back to characters user_ids", exc_info=True)
...
async def rename_character_global(
    self,
    guild_id: int,
    user_id: int,
    old_name: str,
    ...
    """Rename a character while preserving all stats and propagating to related tables.

    Updates characters.name and propagates to any public table that has:
      - character_name column
      - guild_id column
    If user_id column exists on that table, it is also matched.
    Returns number of tables successfully updated (best-effort).
    """
    old_name = (old_name or "").strip()
    new_name = (new_name or "").strip()
    if not old_name or not new_name:
    ...
    await self._require_conn()

    exists = await self._fetchone(
        "SELECT 1 FROM characters WHERE guild_id=%s AND user_id=%s AND lower(name)=lower(%s) LIMIT 1;",
        (guild_id, user_id, new_name),
    )
    if exists:
        raise ValueError("A character with that name already exists for this user in this guild.")

    updated_tables = 0

    async with self._conn.transaction():
        # characters table (authoritative)
        await self._execute(
            "UPDATE characters SET name=%s, updated_at=NOW() WHERE guild_id=%s AND user_id=%s AND name=%s;",
            (new_name, guild_id, user_id, old_name),
        )
        ...
        # discover tables with character_name + guild_id
        col_rows = await self._fetchall(
            """
            SELECT table_name,
                   bool_or(column_name='guild_id') AS has_guild_id,
                   bool_or(column_name='user_id') AS has_user_id
            FROM information_schema.columns
            WHERE table_schema='public'
              AND column_name IN ('character_name','guild_id','user_id')
            GROUP BY table_name
            HAVING bool_or(column_name='character_name') = TRUE
               AND bool_or(column_name='guild_id') = TRUE;
            """,
            (),
        )
        ...
        for r in col_rows:
            ...
            if table == "characters":
                continue
            has_user = bool(r.get("has_user_id", False))
            if has_user:
                where = "guild_id=%s AND user_id=%s AND character_name=%s"
                params = (new_name, guild_id, user_id, old_name)
            else:
                where = "guild_id=%s AND character_name=%s"
                params = (new_name, guild_id, old_name)

            sql = f"UPDATE {table} SET character_name=%s WHERE {where};"
            try:
                await self._execute(sql, params)
                updated_tables += 1
            except Exception:
                LOG.exception("Failed to propagate rename to table %s (continuing)", table)
...
@app_commands.command(name="character_rename", description="Rename a character (preserves stats and linked data).")
@app_commands.describe(character_name="Select the character (autocomplete)", new_name="New character name")
@app_commands.autocomplete(character_name=autocomplete_character_guild)
async def character_rename(self, interaction: discord.Interaction, character_name: str, new_name: str):
    await defer_ephemeral(interaction)
    try:
        ...
        tables_updated = await self.db.rename_character_global(
        ...
        await interaction.followup.send(
            f"✅ Renamed **{old_name}** → **{new_name}**. (Updated {tables_updated} table(s) where applicable.)",
            ephemeral=True,
        )
    except Exception as e:
        await send_error(interaction, e)
